...
std::optional<Distances> gor_with_cycles(Graph &G, std::variant<NodeID, const Distances *> source_pot) {
    ...
    for (NodeID i = 0; i < n; i++) {
        edgesOf[i] = G.getEdgesOf(i, Orientation::OUT);
        edgesOf_T[i] = G.getEdgesOf(i, Orientation::IN);
    }
    std::vector<status> Status(n, OUT_OF_STACKS);
    ...
    while (new_pass.size() != 0) {
        ...
        while (new_pass.size() != 0) {
            ...
            if (Status[i] == IN_NEW_PASS) {
                // search for arcs with reduced costs < 0
                bool found = false;
                ...
                for (const Edge &arc_ij: edgesOf[i]) {
                    ...
                    if (dist[i] + pot_edge_w < dist[arc_ij.target] && dist[i] < c::infty) {
                        found = true;
                        break;
                    }
                }
            ...
            while (dfs.size() != 0) {
                ...
                bool found = false;
                for (auto arc_ij = Current[i]; arc_ij != edgesOf[i].end() && !found; ++arc_ij) {
                    j = arc_ij->target;
                    ...
                    if ((dist[i] < c::infty || pot_edge_w == 0) && dist[i] + pot_edge_w <= dist[j] && Status[j] <
                        IN_TOP_SORT) {
                        // && dist[j] != c::infty) {
                        Current[i] = arc_ij;
                        ++Current[i];
        ...
        // Finding negative edges in the SCCs of the admissible graph
        scc_count = 0;
        for (auto it: pass) {
            scc[it] = 0;
            Current_T[it] = edgesOf_T[it].begin();
        }

        for (auto it: std::views::reverse(pass)) {
            if (scc[it] != 0)
                continue;
            ...
            while (dfs.size() != 0) {
                ...
                for (auto arc = Current_T[v]; arc != edgesOf_T[v].end(); ++arc) {
                    ...
                    if (dist[u] == c::infty || Status[u] != IN_PASS)
                        continue;
